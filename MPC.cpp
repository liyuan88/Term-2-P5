 ...
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    // the Solver function below.
      fg[0]=0; //initial cost is 0
      
    //tuning the parameters of the cost function
      for (int i=0; i<N; i++){
      ...
      //Initial the other variables in fg vector
      ...
      for (int t = 1; t < N; t++) {
          ...
          AD<double> df0 = coeffs[1] + 2.0 * coeffs[2] * x0+3 * coeffs[3] * CppAD::pow(x0, 2);
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  for (int i = delta_start; i < a_start; i++) {
      vars_lowerbound[i] = -0.436332; //[-25,25] in radians
